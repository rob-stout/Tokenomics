...
# would clobber all four targets. (No version sed here: NEW_VERSION was read
# from project.yml itself, so rewriting it is a no-op for the app targets and
# would only overwrite the Safari targets' independent versions.)
sed -i '' "s/CFBundleVersion: \"$INFILE_BUILD\"/CFBundleVersion: \"$NEXT_BUILD\"/g" "$YML_PATH"
