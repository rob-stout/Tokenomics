...
# TokenomicsWidgets/Info.plist is regenerated by xcodegen from project.yml —
# include it so the committed file doesn't drift behind the shipped version.
git add "$YML_PATH" "$PROJECT_ROOT/Tokenomics/Resources/Info.plist" \
    "$PROJECT_ROOT/TokenomicsWidgets/Info.plist"
